...
Start by adding the new devices key to the .sops.yaml file. run one of the following commands to find the ssh-key based age key.
...
#+begin_src sh
add the key to .sops.yaml and create a new section for the new host
#+end_src
...
sops updatekeys modules/users/brian/bmg-secrets.yaml
